...
- **Embedding providers**: 16 recipes covering OpenAI (default fallback), OpenRouter, Voyage, ZeroEntropy (default), Google Gemini, Azure OpenAI, MiniMax, Alibaba DashScope, Zhipu, Fireworks, Ollama (local), llama.cpp llama-server (local), LiteLLM proxy. Pricing matrix + decision tree in [`docs/integrations/embedding-providers.md`](docs/integrations/embedding-providers.md).
- **Chat providers**: 20 recipes. Embeddings are only one of four touchpoints — chat, query expansion and the subagent tool loop route separately, so a brain can mix providers per touchpoint. Ollama and llama.cpp `llama-server` serve chat WITH tool calling, which means a brain can run start to finish — ingest, embed, search, `think`, subagents — with no hosted API key anywhere. `gbrain models autotune` reads the models you have pulled and gives each tier one sized for its job. Hosted options are Anthropic, OpenAI, Google, DeepSeek, Groq, Together, Mistral, Moonshot, Zhipu, MiniMax, NVIDIA, xAI, Cerebras, Fireworks, SambaNova and OpenRouter, plus the `claude-cli` and LiteLLM-proxy escape hatches. Setup, trade-offs and the local-model caveats in [`docs/guides/local-models.md`](docs/guides/local-models.md).
